...
export function registerPipelineTools(api, dbPath, log) {
  ...
  api.registerTool({
    ...
    description: `Create a multi-step workflow pipeline. Define steps with dependencies to form a DAG. Steps execute in order based on their dependencies. Supports conditional branching via depends_on with {step, when} objects.

After creating a pipeline, use pipeline_schedule to record the schedule, then set up an openclaw cron job to wake you at the right times:
  exec openclaw cron add --name "<pipeline-name>" --agent main --message "Your <pipeline-name> pipeline is due. Call pipeline_tick to find ready steps, execute each one, then pipeline_advance when done. Repeat until complete." --cron "<expression>" --tz America/New_York --session isolated --deliver

This way you only wake up when there's actual work — no polling.`,
  ...
  api.registerTool({
    ...
    description: `Create a recurring trigger for a pipeline. This records the schedule in the database for visibility in pipeline_status and lilyToday. You must ALSO set up an openclaw cron job to actually wake you at the scheduled time:
  exec openclaw cron add --name "<pipeline-name>" --agent main --message "Your <pipeline-name> pipeline is due. Call pipeline_tick to find ready steps, execute each one, then pipeline_advance when done. Repeat until complete." --cron "<same-expression>" --tz <timezone> --session isolated --deliver
Cron examples: '0 5 * * *' (daily 5AM), '0 8-20 * * 1-5' (hourly 8AM-8PM weekdays), '0 */2 * * *' (every 2h).`,
    parameters: { type: "object", properties: {
    ...
    async execute(_id, params) {
      ...
      const pipelineName = sqliteQuery(dbPath, `SELECT name FROM pipelines WHERE id = ?`, [params.pipeline_id]);
      const name = pipelineName[0]?.name || params.pipeline_id;

      log.info?.(`pipeline: scheduled ${params.pipeline_id} at "${params.schedule}"`);

      const cronCmd = `openclaw cron add --name "${name}" --agent main --message "Your ${name} pipeline is due. Call pipeline_tick to find ready steps, execute each one, then pipeline_advance when done. Repeat until complete." --cron "${params.schedule}" --tz ${tz} --session isolated --deliver`;

      return {
        content: [{ type: "text", text: `Schedule recorded: ${params.schedule} (${tz})\n\nNow set up the wake-up cron. Run this via exec:\n${cronCmd}` }],
        details: result,
      };
    },
  }, { name: "pipeline_schedule" });
